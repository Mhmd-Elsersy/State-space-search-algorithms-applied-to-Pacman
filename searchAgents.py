...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            "*** YOUR CODE HERE ***"
            x,y = state[0]          #get coordinates of current state
            dx, dy = Actions.directionToVector(action)  #get amount of shift to the next node
            nextx, nexty = int(x + dx), int(y + dy)     #add it to get the node
            hitsWall = self.walls[nextx][nexty]     #check if it hits the wall or not
            ...
            if(not hitsWall):
                ...
                successors.append([[next_state, flags],  action, self._expanded])
